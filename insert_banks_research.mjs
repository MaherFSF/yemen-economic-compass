...
async function insertBanks() {
  ...
  console.log(`Inserting ${research.results.length} banks...`);
  ...
  for (const result of research.results) {
    ...
    if (typeEnum === 'exchange') typeEnum = 'exchange';
    if (typeEnum === 'development') typeEnum = 'development';
  ...
  console.log('\nDone! Verifying insertion...');
  const count = await db.query('SELECT COUNT(*) as count FROM banks');
  console.log(`Total banks in database: ${count[0].count}`);
}
