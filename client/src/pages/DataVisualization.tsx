...
// Data will be fetched from database via TRPC
...
export default function DataVisualization() {
  ...
  // Process exchange rate data from database
  const exchangeRateData = useMemo(() => {
    ...
    return years.map(year => {
      ...
      return {
        year: parseInt(year),
        aden: adenYear ? parseFloat(adenYear.value) : 0,
        sanaa: sanaaYear ? parseFloat(sanaaYear.value) : 0,
        event: ""
      };
    });
  }, [indicators]);

  // Process inflation data from database
  const inflationData = useMemo(() => {
    if (!indicators) return [];
    const inflationIndicators = indicators.filter(i => i.nameEn === "Inflation Rate");
    return inflationIndicators.map(i => ({
      year: parseInt(i.date.substring(0, 4)),
      aden: parseFloat(i.value),
      sanaa: parseFloat(i.value) * 0.85, // Estimate
      food: parseFloat(i.value) * 1.2 // Estimate
    })).sort((a, b) => a.year - b.year);
  }, [indicators]);

  // Process GDP data from database
  const gdpData = useMemo(() => {
    if (!indicators) return [];
    const gdpIndicators = indicators.filter(i => i.nameEn === "GDP Nominal");
    return gdpIndicators.map(i => ({
      year: parseInt(i.date.substring(0, 4)),
      nominal: parseFloat(i.value),
      real: parseFloat(i.value) * 0.95, // Estimate
      sector: { oil: 0, services: 0, agriculture: 0, other: 0 }
    })).sort((a, b) => a.year - b.year);
  }, [indicators]);

  // Oil production data - placeholder since we don't have this in database yet
  const oilProductionData = useMemo(() => {
    return exchangeRateData.map(d => ({
      year: d.year,
      production: 0,
      revenue: 0,
      status: ""
